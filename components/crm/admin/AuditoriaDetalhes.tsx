...
function BlocoJson({ titulo, valor }: { titulo: string; valor: unknown }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
      ...
      <pre className="max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-white p-3 text-[11px] leading-relaxed text-slate-700">
        {formatarJson(valor)}
      </pre>
    </div>
  );
...
export default function AuditoriaDetalhes({ log }: AuditoriaDetalhesProps) {
  if (!log) {
    return (
      <aside className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
  ...
  return (
    <aside className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div>
        ...
        <h3 className="mt-1 text-lg font-bold text-slate-900">{log.acao}</h3>
